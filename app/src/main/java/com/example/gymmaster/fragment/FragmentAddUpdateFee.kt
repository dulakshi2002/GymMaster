...
import com.example.gymmaster.databinding.FragmentAddUpdateFeeBinding
...
class FragmentAddUpdateFee : Fragment() {
    ...
    private fun saveData(){
        try{
            val sqlQuery = "INSERT OR REPLACE INTO FEE(ID,ONE_MONTH,THREE_MONTH,SIX_MONTH,ONE_YEAR,THREE_YEAR)" +
                    "('1','" +binding.edtOneMonth.text.toString().trim()+ "','" + binding.edtThreeMonth.text.toString().trim()+ "','"+
                    "'"+binding.edtSixMonth.text.toString().trim()+ "','" +binding.edtOneYear.text.toString().trim()+ "','" +binding.edtThreeYear.text.toString().trim()+ "')"
